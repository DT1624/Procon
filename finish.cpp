...
void init(string tmp, Board &start_board, Board &goal_board, Die_set &die_set){
    ifstream infile(tmp);
...
int count_same(Board start, Board goal, int i, int j) {
    int w = start.width;
    int h = start.height;
    int cnt = 0;
    while(i * w + j < w * h && start.arr[i][j] == goal.arr[i][j]) {
        cnt++;
        int a = i * w + j + 1;
        i = a / w;
        j = a % w;
    ...
    for(int i = 0; i < m * n; ++i) {
        ...
        if(ans.size() > 0) {
            ...
            for(auto c : ans) {
                ...
                Answer answer1;
                init_Steps(board, answer1, j, k, c.first, c.second);
                int tmp = chuoi[abs(c.first - j)] + chuoi[abs(c.second - k)] - count_same(board, goal_board, j, k);
                if(tmp < cnt_mn) {
                    cnt_mn = tmp;
                    p = c;
...
int main() {
    ...
    for(int i = 47; i < 56; ++i) {
        ...
        string link = "input" + to_string(i) +".txt";
        if(fileExists(link)) {
            init(link, start_board, goal_board, die_set);
        ...
        //    }
            ...
            ofstream f("output" + to_string(i) + ".txt");
